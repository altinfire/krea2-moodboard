...
- **Search** (`/` focuses it) matches title, keywords, and taste profile. **Dropdowns**
  filter by medium / mood / palette / subject, with per-value counts. Checkboxes narrow to
  the 24 staff picks or your favorites.
...
- **Click a thumbnail** for a lightbox (arrow keys navigate, Esc closes). **Click a facet
  label** on a card to filter by it. **Click the heart** to favorite. Favorites and the tray
...
Sizes 32 / 128 / 256 / 512 / 1024 all exist (~0.2 / 0.7 / 2 / 19 / 27 KB avg). The catalog
bundles the **512px** previews (~253 MB) rather than hotlinking the CDN, so visitors never
...
| `krea2-moodboards.json` | Catalog: slug, title, keywords, profile, staffPick, facets, image paths. Regenerated — do not hand-edit. |
| `krea2-moodboard-facets.json` | Facet labels per slug **with provenance** (`source`: `consensus` / `arbiter`). This is the file the classification pipeline owns; the build script merges it into the catalog by slug. |
